//
//
// This file was automatically generated by generateAst
// at 2022-06-26 12:32:01 AM UTC. Do not modify.
//
//

using System.Collections.Generic;

using Trs80.Level1Basic.VirtualMachine.Exceptions;
using Trs80.Level1Basic.VirtualMachine.Parser.Expressions;

namespace Trs80.Level1Basic.VirtualMachine.Parser.Statements;

public class On : Statement
{
    public Selector Selector { get; init; }
    public bool HasRedirector { get; init; }
    public List<Expression> Locations { get; init; }
    public List<int> LinePositions { get; init; }
    public bool IsGosub { get; init; }

    public On(Selector selector, bool hasRedirector, List<Expression> locations, List<int> linePositions, bool isGosub)
    {
        Selector = selector;
        HasRedirector = hasRedirector;
        Locations = locations;
        LinePositions = linePositions;
        IsGosub = isGosub;
    }

    public override T Accept<T>(IVisitor<T> visitor)
    {
        CheckExceptions();
        return visitor.VisitOnStatement(this);
    }
}
