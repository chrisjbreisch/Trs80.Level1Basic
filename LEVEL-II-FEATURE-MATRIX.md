...
| Remaining Level II commands | Partial | All VM-backed Level II command keywords are now inventoried with focused scanner/parser coverage. `ON ERROR GO TO`, `ON ERROR GO TO 0`, and `RESUME` are accepted as statement forms; runtime error-handler dispatch remains separate. `BEEP` produces a short host tone; `OUT` and `WAIT` are intentionally silent, non-blocking no-ops because no TRS-80 port devices are emulated; `LPRINT` and `LLIST` invoke the host printer interface. `SYSTEM`, `SET`, `RESET`, and `POKE` dispatch to existing lifecycle, graphics, and memory APIs. `AUTO` and `EDIT` are handled by interactive input; `CLOAD` and `CSAVE` are file-backed aliases for `LOAD` and `SAVE`, not cassette emulation. | Add runtime `ON ERROR` dispatch and richer editor forms, then revisit remaining command abbreviations. The parser statement dispatch is centralized in `Parser.Statement()`. |
...
322. Added parser and statement coverage for `ON ERROR GO TO`, `ON ERROR GO TO 0`, and `RESUME`; runtime error-handler dispatch remains open.
## Next Slice Queue
