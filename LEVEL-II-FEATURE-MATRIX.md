...
| Control flow | Implemented | Existing `FOR/NEXT`, `GOSUB/RETURN`, `IF`, `ON`, and branch forms are available. Program `STOP` reports `BREAK IN line` and prints `READY` on the following line without an intervening blank line; `CONT` resumes at the next statement. | Audit remaining Level II-specific syntax and error behavior. Existing anchors: `FlowControlTest`, `LogicalTest`, `Level2CompatibilityTest`. |
