...
| Latest implementation commit | `1ec9a05 Record Phase 3 closure checkpoint` |
| Latest documentation checkpoint | Phase 4 dashboard and next-slice queue |
| Validation baseline | Full solution: `729 passed`; interpreter project: `653 passed` |
...
| Typed scalar assignment | Partial | Integer, single, double, and string targets cast assigned values. Double values retain precision through division and conversion functions. Assigning a double to a single target narrows to single precision; assigning that single value to a double target widens the stored single value without restoring lost precision. | Define remaining invalid-conversion and profile-specific promotion rules. Add a dedicated type-conversion test class when the rules are settled. |
| `DIM` arrays | Implemented | One- and two-dimensional arrays are supported. Implicit arrays create on first use with numeric zero defaults; declared upper bounds and negative-subscript checks are enforced. | Verify higher dimensions, redeclaration, implicit-array rules, and exact error behavior. Existing anchors: `ExpressionTest` array tests. |
| Typed arrays | Partial | Numeric and string element casting exists. Explicit array suffixes override conflicting `DEF*` defaults. Implicit numeric and string arrays create on first use; untouched string elements default to empty strings and numeric elements default to zero. Duplicate `DIM` declarations are rejected with `HOW?`, and `CLEAR` resets one- and two-dimensional array values while preserving their bounds. | Verify higher dimensions, array type changes, mixed suffixes, and bounds after `CLEAR`. |
...
| Area | Status | Implemented now | Remaining work and evidence |
| --- | --- | --- | --- |
| Program editing commands | Partial | `LIST` with closed, open-ended, or reversed line ranges, `LOAD`, `SAVE`, `MERGE`, `CLEAR`, and explicit single-line, closed-range, or open-ended `DELETE` exist; `CLEAR length` sets the active string-input capacity from 0 through 255 while preserving the program. `LIST` paginates after 12 lines when another page remains; `Enter` and `UpArrow` continue to the next page, and `READY` appears only after listing completes. Bare `LOAD` and `SAVE` use the Windows open/save dialogs, while quoted paths remain available for scripts; cancelling either file dialog is a no-op, preserving the current program without a success message. `DEL.`, `LO.`, `ME.`, and `SA.` are accepted as DELETE, LOAD, MERGE, and SAVE abbreviations. `CLEAR` resets variables and arrays while preserving the program. Plain `AUTO` with optional start/increment and plain `EDIT line` are handled by the line editor. `CLOAD` and `CSAVE` are accepted as aliases for file-backed `LOAD` and `SAVE`; cassette transport is not emulated. The modern `EDIT` experience intentionally differs from the original TRS-80 keyboard workflow because the original keyboard had no arrow keys. | Verify remaining command abbreviations and richer editor forms. Existing anchor: `CommandTest`, `FileTest`. |
| Data statements | Implemented | `DATA`, `READ`, and `RESTORE` exist. `RESTORE line` restarts at the first DATA statement at or after the requested line. READ source and declared scalar or array target types must both be string or both numeric; either mismatch direction reports `?TM ERROR`. Explicit `$` and implicit `DEFSTR` targets accept string DATA, and reading beyond the available DATA elements reports `?OD ERROR` at the unread target. | Keep exhaustion, explicit-target, and `CLEAR` lifecycle cases as regression anchors. Existing anchor: `DataTest`. |
| Console input/output | Implemented | `INPUT`, `PRINT`, `TAB`, `SPC`, and cursor-related behavior exist; real-console prompts use `? ` spacing, and a missing comma-separated value produces a second `?? ` prompt. The test harness captures prompt writes contiguously, so its equivalent buffer is `? ?? `. Comma-separated variable input such as `INPUT D,N` consumes available values from one response line and prompts separately for missing values, while semicolon-separated variables retain independent reads. Typed input overflow routes through the active error policy. | Audit remaining formatting, input errors, and Level II line-width behavior. Existing anchors: `InputTest`, `PrintTest`, `NativeFunctionTest`. |
...
| Remaining Level II commands | Partial | All VM-backed Level II command keywords are now inventoried with focused scanner/parser coverage. `ON ERROR GO TO` redirects runtime program errors to the configured handler, `ON ERROR GO TO 0` disables trapping, and `RESUME` continues after the faulting statement. `BEEP` produces a short host tone; `OUT` and `WAIT` are intentionally silent, non-blocking no-ops because no TRS-80 port devices are emulated; `LPRINT` and `LLIST` invoke the host printer interface. `SYSTEM`, `SET`, `RESET`, and `POKE` dispatch to existing lifecycle, graphics, and memory APIs. `AUTO` and `EDIT` are handled by interactive input; `CLOAD` and `CSAVE` are file-backed aliases for `LOAD` and `SAVE`, not cassette emulation. | Add richer editor forms and revisit remaining command abbreviations. The parser statement dispatch is centralized in `Parser.Statement()`. |
...
322. Added parser and statement coverage for `ON ERROR GO TO`, `ON ERROR GO TO 0`, and `RESUME`.
323. Implemented runtime `ON ERROR` redirection and `RESUME` continuation with division-by-zero compatibility coverage.
...
337. Completed Phase 3 profile-aware runtime semantics, diagnostics, native-function availability policy, and host-policy documentation with cross-profile regression coverage.
338. Verified mixed scalar promotion: double-to-single assignment narrows precision and single-to-double assignment preserves the stored single value.
339. Added `CLEAR length` string-capacity enforcement, typed integer INPUT overflow routing, targeted `RESUME line`, and focused coverage for the two-input validation flow.
...
349. Added `PRINT USING` numeric image formatting for `#`, decimal precision, `$$` currency placement, comma grouping, and basic `!`/`%%`/`%...%` string fields with focused currency coverage.
...
355. Preserved comma-zone padding inside `PRINT USING`, so multiple values share the image correctly without printing the image text itself.
## Next Slice Queue
