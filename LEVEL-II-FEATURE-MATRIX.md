...
| Data statements | Implemented | `DATA`, `READ`, and `RESTORE` exist. Reading string DATA into a numeric variable reports `?TM ERROR` instead of silently coercing the value to zero. | Add additional mixed-type, exhaustion, and `CLEAR` interaction cases. Existing anchor: `DataTest`. |
