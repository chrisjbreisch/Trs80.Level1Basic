...
| Typed arrays | Partial | Numeric and string element casting exists. Untouched string elements default to empty strings; numeric elements default to zero. Duplicate `DIM` declarations are rejected with `HOW?`, and `CLEAR` resets one- and two-dimensional array values while preserving their bounds. | Verify higher dimensions, implicit creation, array type changes, mixed suffixes, and bounds after `CLEAR`. |
